"""Generate images for Study Partners series.

Hana - Competitive tsundere academic rival.
Uses the hardened MANHWA_STYLE_LOCK for consistent BabeChat-competitive visuals.

This script generates:
1. Hana's character avatar (manhwa style - academic rival)
2. Series cover image (manhwa style)
3. Episode background images (5 episodes - manhwa style)

Usage:
    cd substrate-api/api/src
    REPLICATE_API_TOKEN='...' python -m app.scripts.generate_study_partners_images
    REPLICATE_API_TOKEN='...' python -m app.scripts.generate_study_partners_images --avatar-only
    REPLICATE_API_TOKEN='...' python -m app.scripts.generate_study_partners_images --backgrounds-only
...
# =============================================================================
# MANHWA STYLE CONSTANTS - Hana (Academic Rival)
# =============================================================================

HANA_APPEARANCE = """manhwa style beautiful young woman, intense academic rival, sharp intelligent eyes with competitive fire,
sleek black hair in high ponytail showing elegant neck, delicate features with determined expression,
wearing crisp white blouse with rolled sleeves and fitted skirt, pen tucked behind ear,
arms crossed in challenging pose, the brilliant rival whose attention feels like a prize,
tsundere energy radiating from every glance"""

HANA_STYLE = f"""{MANHWA_STYLE_LOCK['style']}, {MANHWA_STYLE_LOCK['rendering']},
school romance manhwa aesthetic, focused library lighting, intellectual tension"""

HANA_QUALITY = MANHWA_STYLE_LOCK['quality']
HANA_NEGATIVE = MANHWA_STYLE_LOCK['negative']

# Series cover - Hana in library
SERIES_COVER_PROMPT = f"""korean webtoon illustration, manhwa art style, beautiful young woman academic rival in library,
surrounded by stacks of books at study table, looking up from notes with challenging smirk,
one eyebrow raised in competitive taunt, pen in hand ready to prove a point,
school romance manhwa aesthetic, warm golden library lighting, intellectual romantic tension,
...
async def generate_avatar(db: Database, storage: StorageService, image_service: ImageService):
    """Generate Hana's manhwa style avatar."""
    print("\n" + "=" * 60)
    print("GENERATING HANA AVATAR (MANHWA STYLE)")
    print("=" * 60)

    char = await db.fetch_one(
        "SELECT id, name, active_avatar_kit_id, avatar_url FROM characters WHERE slug = 'hana'"
    )
    if not char:
        print("ERROR: Hana character not found!")
        return False

    kit_id = char["active_avatar_kit_id"]
    if not kit_id:
        print("ERROR: No avatar kit found for Hana!")
        return False

    if char["avatar_url"]:
        print(f"Hana already has an avatar: {char['avatar_url'][:80]}...")
        print("Skipping avatar generation.")
        return True

    try:
        full_prompt = f"{HANA_APPEARANCE}, {HANA_STYLE}, {HANA_QUALITY}"
        negative_prompt = HANA_NEGATIVE
...
async def generate_series_cover(db: Database, storage: StorageService, image_service: ImageService):
    """Generate Study Partners series cover in manhwa style."""
    print("\n" + "=" * 60)
    print("GENERATING SERIES COVER (MANHWA STYLE)")
    print("=" * 60)

    series = await db.fetch_one(
        "SELECT id, title, cover_image_url FROM series WHERE slug = 'study-partners'"
    )
    if not series:
...
async def generate_episode_backgrounds(db: Database, storage: StorageService, image_service: ImageService):
    ...
    series = await db.fetch_one(
        "SELECT id FROM series WHERE slug = 'study-partners'"
    )
    if not series:
        print("ERROR: Series not found!")
        return False
    ...
    for ep in episodes:
        ...
        config = STUDY_PARTNERS_BACKGROUNDS.get(title)
        if not config:
...
async def activate_content(db: Database):
    ...
    await db.execute(
        "UPDATE characters SET status = 'active' WHERE slug = 'hana'"
    )
    print("  ✓ Hana character activated")

    await db.execute(
        "UPDATE series SET status = 'active' WHERE slug = 'study-partners'"
    )
    print("  ✓ Study Partners series activated")

    await db.execute(
        """UPDATE episode_templates SET status = 'active'
           WHERE series_id = (SELECT id FROM series WHERE slug = 'study-partners')"""
    )
...
async def main(avatar_only: bool = False, backgrounds_only: bool = False, skip_activation: bool = False):
    ...
    print("=" * 60)
    print("STUDY PARTNERS IMAGE GENERATION")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Generate Study Partners images (MANHWA style)")
