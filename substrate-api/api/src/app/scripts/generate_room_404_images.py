"""Generate images for Room 404 series.

Nina - Strict RA (Southeast Asian beauty) with hidden softness.
Uses MANHWA rendering style with diverse character appearance.

This script generates:
1. Nina's character avatar (manhwa style - Southeast Asian RA)
2. Series cover image (manhwa style)
3. Episode background images (5 episodes - manhwa style)

Usage:
    cd substrate-api/api/src
    REPLICATE_API_TOKEN='...' python -m app.scripts.generate_room_404_images
    REPLICATE_API_TOKEN='...' python -m app.scripts.generate_room_404_images --avatar-only
    REPLICATE_API_TOKEN='...' python -m app.scripts.generate_room_404_images --backgrounds-only
...
# =============================================================================
# CHARACTER APPEARANCE - Nina (Southeast Asian RA)
# Separated: APPEARANCE (ethnicity/features) vs STYLE (rendering technique)
# =============================================================================

# Nina's distinct Southeast Asian appearance
NINA_APPEARANCE = """beautiful young woman with warm golden-tan skin, soft features with gentle cheekbones,
...
wearing cozy oversized university hoodie over sleep shorts showing long legs, holding clipboard,
stern responsible expression hiding caring warmth, the authority figure you want to get in trouble with"""

# Manhwa RENDERING style (not ethnicity)
NINA_STYLE = f"""{MANHWA_STYLE_LOCK['style']}, {MANHWA_STYLE_LOCK['rendering']},
school romance aesthetic, warm dorm hallway lighting, late night intimacy"""

NINA_QUALITY = MANHWA_STYLE_LOCK['quality']
NINA_NEGATIVE = MANHWA_STYLE_LOCK['negative']

# Series cover - Nina in dorm hallway (Southeast Asian beauty in manhwa style)
SERIES_COVER_PROMPT = f"""webtoon illustration, manhwa art style, beautiful young woman with golden-tan skin and almond eyes RA in university hoodie,
standing in dorm hallway at night with arms crossed, flashlight in hand casting dramatic shadows,
looking at viewer with stern expression that hints at hidden warmth, Southeast Asian features, messy hair from late patrol,
school romance aesthetic, warm amber and blue night color palette, intimate late night atmosphere,
...
async def generate_avatar(db: Database, storage: StorageService, image_service: ImageService):
    """Generate Nina's avatar (Southeast Asian in manhwa rendering style)."""
    print("\n" + "=" * 60)
    print("GENERATING NINA AVATAR (SOUTHEAST ASIAN - MANHWA STYLE)")
    print("=" * 60)

    char = await db.fetch_one(
        "SELECT id, name, active_avatar_kit_id, avatar_url FROM characters WHERE slug = 'nina'"
    )
    if not char:
        print("ERROR: Nina character not found!")
        return False

    kit_id = char["active_avatar_kit_id"]
    if not kit_id:
        print("ERROR: No avatar kit found for Nina!")
        return False

    if char["avatar_url"]:
        print(f"Nina already has an avatar: {char['avatar_url'][:80]}...")
        print("Skipping avatar generation.")
        return True

    try:
        full_prompt = f"{NINA_APPEARANCE}, {NINA_STYLE}, {NINA_QUALITY}"
        negative_prompt = NINA_NEGATIVE

        print(f"Generating with prompt: {full_prompt[:150]}...")
        print(f"Style: MANHWA rendering, Southeast Asian character")
...
async def generate_series_cover(db: Database, storage: StorageService, image_service: ImageService):
    """Generate Room 404 series cover in manhwa style."""
    print("\n" + "=" * 60)
    print("GENERATING SERIES COVER (MANHWA STYLE)")
    print("=" * 60)

    series = await db.fetch_one(
        "SELECT id, title, cover_image_url FROM series WHERE slug = 'room-404'"
    )
    if not series:
...
async def generate_episode_backgrounds(db: Database, storage: StorageService, image_service: ImageService):
    ...
    series = await db.fetch_one(
        "SELECT id FROM series WHERE slug = 'room-404'"
    )
    if not series:
        print("ERROR: Series not found!")
        return False
    ...
    for ep in episodes:
        ...
        config = ROOM_404_BACKGROUNDS.get(title)
        if not config:
...
async def activate_content(db: Database):
    ...
    await db.execute(
        "UPDATE characters SET status = 'active' WHERE slug = 'nina'"
    )
    print("  ✓ Nina character activated")

    await db.execute(
        "UPDATE series SET status = 'active' WHERE slug = 'room-404'"
    )
    print("  ✓ Room 404 series activated")

    await db.execute(
        """UPDATE episode_templates SET status = 'active'
           WHERE series_id = (SELECT id FROM series WHERE slug = 'room-404')"""
    )
...
async def main(avatar_only: bool = False, backgrounds_only: bool = False, skip_activation: bool = False):
    ...
    print("=" * 60)
    print("ROOM 404 IMAGE GENERATION")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Generate Room 404 images (MANHWA style)")
