"""Generate images for Academy Secrets series.

This is the EXEMPLAR for manhwa/webtoon style visual direction.
Uses the hardened MANHWA_STYLE_LOCK for consistent BabeChat-competitive visuals.

This script generates:
1. Haru's character avatar (manhwa style - student council president)
2. Series cover image (manhwa style)
3. Episode background images (5 episodes - manhwa style)

Usage:
    cd substrate-api/api/src
    REPLICATE_API_TOKEN='...' python -m app.scripts.generate_academy_secrets_images
    REPLICATE_API_TOKEN='...' python -m app.scripts.generate_academy_secrets_images --avatar-only
    REPLICATE_API_TOKEN='...' python -m app.scripts.generate_academy_secrets_images --backgrounds-only
...
# Import the hardened style lock from avatar_generation
...
# =============================================================================
# MANHWA STYLE CONSTANTS - Hardened Visual Direction
# =============================================================================

# Character appearance using MANHWA style lock
HARU_APPEARANCE = """manhwa style handsome male student, ethereal beauty, soft angular features, sharp intelligent dark eyes behind elegant glasses,
...
# Use the hardened style lock
HARU_STYLE = f"""{MANHWA_STYLE_LOCK['style']}, {MANHWA_STYLE_LOCK['rendering']},
school romance manhwa aesthetic, soft pastel lighting, cherry blossom atmosphere"""

HARU_QUALITY = MANHWA_STYLE_LOCK['quality']
HARU_NEGATIVE = MANHWA_STYLE_LOCK['negative']

# Series cover prompt - Haru at academy in cherry blossom setting
SERIES_COVER_PROMPT = f"""korean webtoon illustration, manhwa art style, handsome male student council president in academy uniform,
standing under cherry blossom tree in prestigious school courtyard, cherry petals floating around him,
looking at viewer with composed expression softening into something real, glasses catching soft light,
school romance manhwa aesthetic, soft pastel pink and white color palette, dreamy romantic atmosphere,
...
async def generate_avatar(db: Database, storage: StorageService, image_service: ImageService):
    """Generate Haru's manhwa style avatar."""
    print("\n" + "=" * 60)
    print("GENERATING HARU AVATAR (MANHWA STYLE)")
    print("=" * 60)

    # Get Haru's character and kit
    char = await db.fetch_one(
        "SELECT id, name, active_avatar_kit_id, avatar_url FROM characters WHERE slug = 'haru'"
    )
    if not char:
        print("ERROR: Haru character not found!")
        return False

    kit_id = char["active_avatar_kit_id"]
    if not kit_id:
        print("ERROR: No avatar kit found for Haru!")
        return False

    # Check if already has an avatar
    if char["avatar_url"]:
        print(f"Haru already has an avatar: {char['avatar_url'][:80]}...")
        print("Skipping avatar generation.")
        return True

    try:
        # Build the prompt using hardened manhwa style lock
        full_prompt = f"{HARU_APPEARANCE}, {HARU_STYLE}, {HARU_QUALITY}"
        negative_prompt = HARU_NEGATIVE
...
async def generate_series_cover(db: Database, storage: StorageService, image_service: ImageService):
    """Generate Academy Secrets series cover in manhwa style."""
    print("\n" + "=" * 60)
    ...
    series = await db.fetch_one(
        "SELECT id, title, cover_image_url FROM series WHERE slug = 'academy-secrets'"
    )
    if not series:
...
async def generate_episode_backgrounds(db: Database, storage: StorageService, image_service: ImageService):
    ...
    series = await db.fetch_one(
        "SELECT id FROM series WHERE slug = 'academy-secrets'"
    )
    if not series:
        print("ERROR: Series not found!")
        return False
    ...
    for ep in episodes:
        ...
        config = ACADEMY_SECRETS_BACKGROUNDS.get(title)
        if not config:
        ...
        try:
            # Build prompt using hardened manhwa style
...
async def activate_content(db: Database):
    ...
    # Activate character
    await db.execute(
        "UPDATE characters SET status = 'active' WHERE slug = 'haru'"
    )
    print("  ✓ Haru character activated")

    # Activate series
    await db.execute(
        "UPDATE series SET status = 'active' WHERE slug = 'academy-secrets'"
    )
    print("  ✓ Academy Secrets series activated")

    # Activate episodes
    await db.execute(
        """UPDATE episode_templates SET status = 'active'
           WHERE series_id = (SELECT id FROM series WHERE slug = 'academy-secrets')"""
    )
...
async def main(avatar_only: bool = False, backgrounds_only: bool = False, skip_activation: bool = False):
    ...
    print("=" * 60)
    print("ACADEMY SECRETS IMAGE GENERATION")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Generate Academy Secrets images (MANHWA style)")
