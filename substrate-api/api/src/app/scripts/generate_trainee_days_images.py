"""Generate images for Trainee Days series.

This script generates:
1. Tae-min's character avatar (raw trainee documentary style)
2. Series cover image
3. Episode background images (6 episodes)

Usage:
    cd substrate-api/api/src
    REPLICATE_API_TOKEN='...' python -m app.scripts.generate_trainee_days_images
    REPLICATE_API_TOKEN='...' python -m app.scripts.generate_trainee_days_images --avatar-only
    REPLICATE_API_TOKEN='...' python -m app.scripts.generate_trainee_days_images --backgrounds-only
"""
...
async def generate_avatar(db: Database, storage: StorageService, image_service: ImageService):
    ...
    # Get Tae-min's character and kit
    char = await db.fetch_one(
        "SELECT id, name, active_avatar_kit_id, avatar_url FROM characters WHERE slug = 'tae-min'"
    )
    if not char:
        print("ERROR: Tae-min character not found! Run scaffold_trainee_days.py first.")
        return False

    kit_id = char["active_avatar_kit_id"]
    if not kit_id:
        print("ERROR: No avatar kit found for Tae-min!")
        return False

    # Check if already has an avatar
    if char["avatar_url"]:
        print(f"Tae-min already has an avatar: {char['avatar_url'][:80]}...")
        print("Skipping avatar generation.")
        return True

    try:
        # Build the prompt
        full_prompt = f"{TAEMIN_APPEARANCE}, {TAEMIN_STYLE}, {TRAINEE_QUALITY}"
...
async def generate_series_cover(db: Database, storage: StorageService, image_service: ImageService):
    """Generate Trainee Days series cover."""
    print("\n" + "=" * 60)
    ...
    series = await db.fetch_one(
        "SELECT id, title, cover_image_url FROM series WHERE slug = 'trainee-days'"
    )
    if not series:
        print("ERROR: Series not found! Run scaffold_trainee_days.py first.")
        return False
...
async def generate_episode_backgrounds(db: Database, storage: StorageService, image_service: ImageService):
    ...
    series = await db.fetch_one(
        "SELECT id FROM series WHERE slug = 'trainee-days'"
    )
    if not series:
        print("ERROR: Series not found! Run scaffold_trainee_days.py first.")
        return False
    ...
    for ep in episodes:
        ...
        config = TRAINEE_DAYS_BACKGROUNDS.get(title)
        if not config:
...
async def activate_content(db: Database):
    ...
    # Activate character
    await db.execute(
        "UPDATE characters SET status = 'active' WHERE slug = 'tae-min'"
    )
    print("  ✓ Tae-min character activated")

    # Activate series
    await db.execute(
        "UPDATE series SET status = 'active' WHERE slug = 'trainee-days'"
    )
    print("  ✓ Trainee Days series activated")

    # Activate episodes
    await db.execute(
        """UPDATE episode_templates SET status = 'active'
           WHERE series_id = (SELECT id FROM series WHERE slug = 'trainee-days')"""
    )
...
async def main(avatar_only: bool = False, backgrounds_only: bool = False, skip_activation: bool = False):
    ...
    print("=" * 60)
    print("TRAINEE DAYS IMAGE GENERATION")
    print("=" * 60)
    print("Style: Raw documentary K-pop trainee aesthetic")
    print("NOT: Polished idol glamour")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Generate Trainee Days images")
