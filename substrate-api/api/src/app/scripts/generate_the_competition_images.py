"""Generate images for The Competition series.

This script generates:
1. Claire's character avatar (cozy bakery owner style)
2. Series cover image
3. Episode background images (5 episodes)

Usage:
    cd substrate-api/api/src
    REPLICATE_API_TOKEN='...' python -m app.scripts.generate_the_competition_images
    REPLICATE_API_TOKEN='...' python -m app.scripts.generate_the_competition_images --avatar-only
    REPLICATE_API_TOKEN='...' python -m app.scripts.generate_the_competition_images --backgrounds-only
...
# Claire's appearance for avatar generation - confident bakery owner
CLAIRE_APPEARANCE = """beautiful woman late 20s, warm brown eyes with competitive spark, wavy auburn hair loosely tied back with flour-dusted strands escaping,
light freckles across nose and cheeks, confident knowing smile with hint of challenge, wearing fitted cream apron over casual sweater with rolled sleeves,
flour on hands, standing in doorway of charming bakery, warm morning light"""

CLAIRE_STYLE = """digital illustration, cozy romance novel cover style, warm golden lighting,
small town charm, inviting bakery atmosphere, single character portrait, soft focus background"""

# Series cover prompt - Claire in her bakery with morning light
SERIES_COVER_PROMPT = """digital illustration, beautiful auburn-haired woman in bakery apron standing in doorway of charming small town bakery,
warm morning sunlight streaming in, fresh pastries visible on display, competitive confident smile, arms crossed,
cozy small town main street visible through windows, autumn leaves, warm inviting atmosphere,
masterpiece, best quality, highly detailed, romance novel aesthetic"""
...
async def generate_avatar(db: Database, storage: StorageService, image_service: ImageService):
    """Generate Claire's cozy romance avatar."""
    print("\n" + "=" * 60)
    print("GENERATING CLAIRE AVATAR")
    print("=" * 60)

    # Get Claire's character and kit
    char = await db.fetch_one(
        "SELECT id, name, active_avatar_kit_id, avatar_url FROM characters WHERE slug = 'claire'"
    )
    if not char:
        print("ERROR: Claire character not found!")
        return False

    kit_id = char["active_avatar_kit_id"]
    if not kit_id:
        print("ERROR: No avatar kit found for Claire!")
        return False

    # Check if already has an avatar
    if char["avatar_url"]:
        print(f"Claire already has an avatar: {char['avatar_url'][:80]}...")
        print("Skipping avatar generation.")
        return True

    try:
        # Build the prompt
        full_prompt = f"{CLAIRE_APPEARANCE}, {CLAIRE_STYLE}, {COZY_QUALITY}"
...
async def generate_series_cover(db: Database, storage: StorageService, image_service: ImageService):
    """Generate The Competition series cover."""
    print("\n" + "=" * 60)
    ...
    series = await db.fetch_one(
        "SELECT id, title, cover_image_url FROM series WHERE slug = 'the-competition'"
    )
    if not series:
...
async def generate_episode_backgrounds(db: Database, storage: StorageService, image_service: ImageService):
    ...
    series = await db.fetch_one(
        "SELECT id FROM series WHERE slug = 'the-competition'"
    )
    if not series:
        print("ERROR: Series not found!")
        return False
    ...
    for ep in episodes:
        ...
        config = THE_COMPETITION_BACKGROUNDS.get(title)
        if not config:
...
async def activate_content(db: Database):
    ...
    # Activate character
    await db.execute(
        "UPDATE characters SET status = 'active' WHERE slug = 'claire'"
    )
    print("  ✓ Claire character activated")

    # Activate series
    await db.execute(
        "UPDATE series SET status = 'active' WHERE slug = 'the-competition'"
    )
    print("  ✓ The Competition series activated")

    # Activate episodes
    await db.execute(
        """UPDATE episode_templates SET status = 'active'
           WHERE series_id = (SELECT id FROM series WHERE slug = 'the-competition')"""
    )
...
async def main(avatar_only: bool = False, backgrounds_only: bool = False, skip_activation: bool = False):
    ...
    print("=" * 60)
    print("THE COMPETITION IMAGE GENERATION")
    print("=" * 60)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Generate The Competition images")
