"""Generate images for Second Chance series.

This script generates:
1. Liam's character avatar (bittersweet reunion style)
2. Series cover image
3. Episode background images (5 episodes)

Usage:
    cd substrate-api/api/src
    REPLICATE_API_TOKEN='...' python -m app.scripts.generate_second_chance_images
    REPLICATE_API_TOKEN='...' python -m app.scripts.generate_second_chance_images --avatar-only
    REPLICATE_API_TOKEN='...' python -m app.scripts.generate_second_chance_images --backgrounds-only
...
# Liam's appearance for avatar generation - bittersweet, carrying weight
LIAM_APPEARANCE = """handsome man early 30s, thoughtful hazel eyes with depth and warmth, dark wavy hair slightly tousled, light stubble,
wearing well-fitted dark suit with loosened tie, standing at elegant venue at dusk, expression mixing hope and uncertainty,
someone carrying years of regret and longing, warm but guarded"""

LIAM_STYLE = """digital illustration, elegant romance novel style, soft romantic lighting,
emotional intimacy, single character portrait, warm evening atmosphere, bittersweet mood"""

# Series cover prompt - Liam at wedding venue at golden hour
SERIES_COVER_PROMPT = """digital illustration, handsome man in dark suit with loosened tie standing at elegant outdoor wedding venue,
golden hour sunset light, fairy lights beginning to glow in background, looking off to the side with mix of hope and uncertainty,
romantic bittersweet atmosphere, years of longing in his expression, elegant venue with flowers,
masterpiece, best quality, highly detailed, elegant romance novel aesthetic"""
...
async def generate_avatar(db: Database, storage: StorageService, image_service: ImageService):
    """Generate Liam's reunion style avatar."""
    print("\n" + "=" * 60)
    print("GENERATING LIAM AVATAR")
    print("=" * 60)

    # Get Liam's character and kit
    char = await db.fetch_one(
        "SELECT id, name, active_avatar_kit_id, avatar_url FROM characters WHERE slug = 'liam'"
    )
    if not char:
        print("ERROR: Liam character not found!")
        return False

    kit_id = char["active_avatar_kit_id"]
    if not kit_id:
        print("ERROR: No avatar kit found for Liam!")
        return False

    # Check if already has an avatar
    if char["avatar_url"]:
        print(f"Liam already has an avatar: {char['avatar_url'][:80]}...")
        print("Skipping avatar generation.")
        return True

    try:
        # Build the prompt
        full_prompt = f"{LIAM_APPEARANCE}, {LIAM_STYLE}, {REUNION_QUALITY}"
...
async def generate_series_cover(db: Database, storage: StorageService, image_service: ImageService):
    """Generate Second Chance series cover."""
    print("\n" + "=" * 60)
    ...
    series = await db.fetch_one(
        "SELECT id, title, cover_image_url FROM series WHERE slug = 'second-chance'"
    )
    if not series:
...
async def generate_episode_backgrounds(db: Database, storage: StorageService, image_service: ImageService):
    ...
    series = await db.fetch_one(
        "SELECT id FROM series WHERE slug = 'second-chance'"
    )
    if not series:
        print("ERROR: Series not found!")
        return False
    ...
    for ep in episodes:
        ...
        config = SECOND_CHANCE_BACKGROUNDS.get(title)
        if not config:
...
async def activate_content(db: Database):
    ...
    # Activate character
    await db.execute(
        "UPDATE characters SET status = 'active' WHERE slug = 'liam'"
    )
    print("  ✓ Liam character activated")

    # Activate series
    await db.execute(
        "UPDATE series SET status = 'active' WHERE slug = 'second-chance'"
    )
    print("  ✓ Second Chance series activated")

    # Activate episodes
    await db.execute(
        """UPDATE episode_templates SET status = 'active'
           WHERE series_id = (SELECT id FROM series WHERE slug = 'second-chance')"""
    )
...
async def main(avatar_only: bool = False, backgrounds_only: bool = False, skip_activation: bool = False):
    ...
    print("=" * 60)
    print("SECOND CHANCE IMAGE GENERATION")
    print("=" * 60)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Generate Second Chance images")
