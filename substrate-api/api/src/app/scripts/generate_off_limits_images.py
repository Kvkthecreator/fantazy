"""Generate images for Off Limits series.

This script generates:
1. Marcus's character avatar (warm hometown style)
2. Series cover image
3. Episode background images (5 episodes)

Usage:
    cd substrate-api/api/src
    REPLICATE_API_TOKEN='...' python -m app.scripts.generate_off_limits_images
    REPLICATE_API_TOKEN='...' python -m app.scripts.generate_off_limits_images --avatar-only
    REPLICATE_API_TOKEN='...' python -m app.scripts.generate_off_limits_images --backgrounds-only
...
# =============================================================================
# Hometown Romance Style Constants
# =============================================================================
...
# Marcus's appearance for avatar generation - rugged but warm older brother type
MARCUS_APPEARANCE = """handsome man early 30s, strong jaw with light stubble, warm brown eyes with guarded depth, short dark hair with natural texture,
broad shoulders, wearing simple dark henley shirt with sleeves pushed up muscular forearms, standing on porch of family home,
warm golden evening light, quiet intensity mixed with protectiveness, someone who's been away too long"""

MARCUS_STYLE = """digital illustration, warm romance novel style, naturalistic golden hour lighting,
intimate domestic atmosphere, single character portrait, family home background soft focus"""

# Series cover prompt - Marcus on family home porch at golden hour
SERIES_COVER_PROMPT = """digital illustration, handsome man early 30s with stubble and broad shoulders in dark henley,
leaning against family home porch railing at golden hour, warm sunset light casting long shadows,
looking off to the side with guarded but intense expression, nostalgic small town atmosphere,
comfortable lived-in porch with string lights, protective stance, someone carrying weight,
masterpiece, best quality, highly detailed, warm naturalistic romance novel aesthetic"""
...
async def generate_avatar(db: Database, storage: StorageService, image_service: ImageService):
    """Generate Marcus's hometown romance avatar."""
    print("\n" + "=" * 60)
    print("GENERATING MARCUS AVATAR")
    print("=" * 60)

    # Get Marcus's character and kit
    char = await db.fetch_one(
        "SELECT id, name, active_avatar_kit_id, avatar_url FROM characters WHERE slug = 'marcus'"
    )
    if not char:
        print("ERROR: Marcus character not found!")
        return False

    kit_id = char["active_avatar_kit_id"]
    if not kit_id:
        print("ERROR: No avatar kit found for Marcus!")
        return False

    # Check if already has an avatar
    if char["avatar_url"]:
        print(f"Marcus already has an avatar: {char['avatar_url'][:80]}...")
        print("Skipping avatar generation.")
        return True

    try:
        # Build the prompt
        full_prompt = f"{MARCUS_APPEARANCE}, {MARCUS_STYLE}, {HOMETOWN_QUALITY}"
...
async def generate_series_cover(db: Database, storage: StorageService, image_service: ImageService):
    """Generate Off Limits series cover."""
    print("\n" + "=" * 60)
    ...
    series = await db.fetch_one(
        "SELECT id, title, cover_image_url FROM series WHERE slug = 'off-limits'"
    )
    if not series:
...
async def generate_episode_backgrounds(db: Database, storage: StorageService, image_service: ImageService):
    ...
    series = await db.fetch_one(
        "SELECT id FROM series WHERE slug = 'off-limits'"
    )
    if not series:
        print("ERROR: Series not found!")
        return False
    ...
    for ep in episodes:
        ...
        config = OFF_LIMITS_BACKGROUNDS.get(title)
        if not config:
...
async def activate_content(db: Database):
    ...
    # Activate character
    await db.execute(
        "UPDATE characters SET status = 'active' WHERE slug = 'marcus'"
    )
    print("  ✓ Marcus character activated")

    # Activate series
    await db.execute(
        "UPDATE series SET status = 'active' WHERE slug = 'off-limits'"
    )
    print("  ✓ Off Limits series activated")

    # Activate episodes
    await db.execute(
        """UPDATE episode_templates SET status = 'active'
           WHERE series_id = (SELECT id FROM series WHERE slug = 'off-limits')"""
    )
...
async def main(avatar_only: bool = False, backgrounds_only: bool = False, skip_activation: bool = False):
    ...
    print("=" * 60)
    print("OFF LIMITS IMAGE GENERATION")
    print("=" * 60)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Generate Off Limits images")
