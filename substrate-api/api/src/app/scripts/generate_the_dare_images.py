"""Generate images for The Dare series.

Bella - Queen Bee (Mediterranean beauty) with party dare gone real.
Uses MANHWA rendering style with diverse character appearance.

This script generates:
1. Bella's character avatar (manhwa style - Mediterranean queen bee)
2. Series cover image (manhwa style)
3. Episode background images (5 episodes - manhwa style)

Usage:
    cd substrate-api/api/src
    REPLICATE_API_TOKEN='...' python -m app.scripts.generate_the_dare_images
    REPLICATE_API_TOKEN='...' python -m app.scripts.generate_the_dare_images --avatar-only
    REPLICATE_API_TOKEN='...' python -m app.scripts.generate_the_dare_images --backgrounds-only
...
# =============================================================================
# CHARACTER APPEARANCE - Bella (Mediterranean Queen Bee)
# Separated: APPEARANCE (ethnicity/features) vs STYLE (rendering technique)
# =============================================================================

# Bella's distinct Mediterranean appearance
BELLA_APPEARANCE = """beautiful young woman with warm olive skin, thick wavy dark brown hair flowing past shoulders,
expressive hazel-brown eyes with golden flecks, confident knowing smile,
Mediterranean features with elegant brows, natural warmth in expression,
wearing stylish casual outfit with designer accessories, confident relaxed pose,
radiating magnetic charm and effortless confidence"""

# Manhwa RENDERING style (not ethnicity)
BELLA_STYLE = f"""{MANHWA_STYLE_LOCK['style']}, {MANHWA_STYLE_LOCK['rendering']},
school romance aesthetic, dramatic party lighting, electric atmosphere"""

BELLA_QUALITY = MANHWA_STYLE_LOCK['quality']
BELLA_NEGATIVE = MANHWA_STYLE_LOCK['negative']

# Series cover - Bella at party (Mediterranean beauty in manhwa style)
SERIES_COVER_PROMPT = f"""webtoon illustration, manhwa art style, beautiful young woman with olive skin and wavy brown hair at house party,
leaning against wall, colorful party lights creating dramatic mood, Mediterranean features,
looking at viewer with confident knowing smile, hazel eyes catching the light,
school romance aesthetic, neon pink and purple color palette, electric party atmosphere,
...
async def generate_avatar(db: Database, storage: StorageService, image_service: ImageService):
    """Generate Bella's avatar (Mediterranean in manhwa rendering style)."""
    print("\n" + "=" * 60)
    print("GENERATING BELLA AVATAR (MEDITERRANEAN - MANHWA STYLE)")
    print("=" * 60)

    char = await db.fetch_one(
        "SELECT id, name, active_avatar_kit_id, avatar_url FROM characters WHERE slug = 'bella'"
    )
    if not char:
        print("ERROR: Bella character not found!")
        return False

    kit_id = char["active_avatar_kit_id"]
    if not kit_id:
        print("ERROR: No avatar kit found for Bella!")
        return False

    if char["avatar_url"]:
        print(f"Bella already has an avatar: {char['avatar_url'][:80]}...")
        print("Skipping avatar generation.")
        return True

    try:
        full_prompt = f"{BELLA_APPEARANCE}, {BELLA_STYLE}, {BELLA_QUALITY}"
        negative_prompt = BELLA_NEGATIVE

        print(f"Generating with prompt: {full_prompt[:150]}...")
        print(f"Style: MANHWA rendering, Mediterranean character")
...
async def generate_series_cover(db: Database, storage: StorageService, image_service: ImageService):
    """Generate The Dare series cover in manhwa style."""
    print("\n" + "=" * 60)
    print("GENERATING SERIES COVER (MANHWA STYLE)")
    print("=" * 60)

    series = await db.fetch_one(
        "SELECT id, title, cover_image_url FROM series WHERE slug = 'the-dare'"
    )
    if not series:
...
async def generate_episode_backgrounds(db: Database, storage: StorageService, image_service: ImageService):
    ...
    series = await db.fetch_one(
        "SELECT id FROM series WHERE slug = 'the-dare'"
    )
    if not series:
        print("ERROR: Series not found!")
        return False
    ...
    for ep in episodes:
        ...
        config = THE_DARE_BACKGROUNDS.get(title)
        if not config:
...
async def activate_content(db: Database):
    ...
    await db.execute(
        "UPDATE characters SET status = 'active' WHERE slug = 'bella'"
    )
    print("  ✓ Bella character activated")

    await db.execute(
        "UPDATE series SET status = 'active' WHERE slug = 'the-dare'"
    )
    print("  ✓ The Dare series activated")

    await db.execute(
        """UPDATE episode_templates SET status = 'active'
           WHERE series_id = (SELECT id FROM series WHERE slug = 'the-dare')"""
    )
...
async def main(avatar_only: bool = False, backgrounds_only: bool = False, skip_activation: bool = False):
    ...
    print("=" * 60)
    print("THE DARE IMAGE GENERATION")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Generate The Dare images (MANHWA style)")
